...
class Temperature extends React.Component {
  ...
  updateActiveUnit = activeUnit => {
    ...
    const index = temperatures.findIndex(unit => {
      if (unit.id && activeUnit.id) {
        return unit.id === activeUnit.id;
      }
      return unit.unit_id === activeUnit.unit_id;
    });
    if (activeUnit.temperature < -3 || activeUnit.temperature > 3)
      activeUnit.temperature = 0;
    if (index !== -1) {
  ...
  deleteActiveUnit = activeUnit => {
    ...
    temperatures = temperatures.filter(
      unit => unit.unit_id !== activeUnit.unit_id
    );
    ...
    return (
      <>
        ...
        <Grid container>
          ...
          <Grid item sm xs={12}>
            <Right
              deleteActiveUnit={this.deleteActiveUnit}
              updateActiveUnit={this.updateActiveUnit}
